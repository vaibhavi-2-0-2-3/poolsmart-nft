...
-- Fix the get_event_rides function to resolve ambiguous column references
...
BEGIN
  ...
  SELECT 
    ...
    r.seats,
    r.price,
    (r.seats - COALESCE(accepted_requests.count, 0)) as available_seats
  FROM public.rides r
  INNER JOIN public.event_rides er ON r.id = er.ride_id
  LEFT JOIN public.profiles p ON r.user_id = p.id
  LEFT JOIN (
    SELECT rr.ride_id, COUNT(*) as count
